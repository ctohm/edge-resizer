...
function checkHeaders(req: RequestWithParams): Promise<Response> {
  ...
  return fetch(requested).then((res: Response) => {
    if (!res.ok) {
      return json(Object.fromEntries((res as Response).headers))
    }
    return res
  })
...
export default {
  fetch: async (request: Request, env: EnvWithBindings, ctx: FetchEvent): Promise<Response> => {
    ...
    // Replace 
    const mainRouter: ThrowableRouter<Request> = ThrowableRouter({ base: '' })
