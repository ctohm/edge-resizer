...
async function computeCachedResponse(imageRequest, ctx, env) {
  let debug = env && env.DEBUG ? console.log.bind("computeCachedResponse:") : () => {
    return null;
  };
...
var thirdParty = async (request, ctx, env) => {
  const { transforms, protocol = "https", domain, pathname, origin } = request.params || {};
  let debug = env && env.DEBUG ? console.log.bind("thirdParty:") : () => {
    return null;
  };
...
var ResizerRouter = class {
  constructor(options) {
    ...
    const ittyRouter = (0, import_itty_router.Router)(ittyOpts);
  ...
  async handleMatchingRoute(req, env, ctx) {
    const url = new URL(req.url), debug = (env || {}).DEBUG || url.searchParams.has("debug") ? console.log.bind("ResizerRouter:") : () => {
      return null;
    };
